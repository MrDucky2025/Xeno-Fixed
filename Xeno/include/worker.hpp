...
// Updated For version-c1ac69007bdc4e48 By ente0216
...
public:
    ...
    inline std::uintptr_t FetchDataModel() const {
        ...
        std::wstring targetModule = L"RobloxPlayerBeta.exe";

        if (Module32First(snapshot, &moduleInfo)) {
            do {
                if (wcscmp(moduleInfo.szModule, targetModule.c_str()) == 0) {
                    CloseHandle(snapshot);

                    uintptr_t RobloxBase = (uintptr_t)(moduleInfo.modBaseAddr);

                    write_memory<BYTE>(RobloxBase + offsets::EnableLoadModule, 1, handle);

                    uintptr_t FakeDatamodel = read_memory<uintptr_t>(RobloxBase + offsets::FakeDatamodelPointer, handle);
                    if (FakeDatamodel == 0) {
                        std::cerr << "Could not read RenderView\n";
                        return 0;
                    }

                    uintptr_t Datamodel = read_memory<uintptr_t>(FakeDatamodel + offsets::FakeToRealDatamodel, handle);
                    if (Datamodel == 0) {
